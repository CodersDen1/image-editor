...
class UserController {
  ...
  async uploadProfileImage(userId, file) {
    try {
      ...
      if (user.profileImage) {
        ...
        if (oldImageKey) {
          await storageService.deleteFile(`users/${userId}/profile/${oldImageKey}`, userId);
        }
      }
  ...
  async deleteProfileImage(userId) {
    try {
      ...
      if (imageKey) {
        await storageService.deleteFile(`users/${userId}/profile/${imageKey}`, userId);
      }
      ...
      return {
        ...
        stats: {
          ...
          storage: {
            used: storageUsed,
            limit: storageLimit,
            percentage: Math.min(storagePercentage, 100).toFixed(2),
            formattedUsed: this._formatBytes(storageUsed),
            formattedLimit: this._formatBytes(storageLimit)
          },
